...
describe('组合口径', () => {
   ...
   * 以及它走**严格配对** —— 影子的基准列真的会缺（真机上 2/2 行都是 null）。
   */
  it('beta：基准缺失或样本不足时为 null，不是 0', () => {
    expect(summarize(base({ equity: equity([['2026-01-05', 1_000_000, null], ['2026-01-06', 1_010_000, null]]) })).beta).toBeNull()
    expect(summarize(base({ equity: equity([['2026-01-05', 1_000_000, 4000]]) })).beta).toBeNull()
  })

  it('beta：净值跟着基准走一半幅度 ⇒ 约 0.5（基准中间缺一天也不许错位）', () => {
    const clean = summarize(
      base({
        equity: equity([
          ['2026-01-05', 1_000_000, 4000],
          ['2026-01-06', 1_020_000, 4160], // 基准 +4%，策略 +2%
          ['2026-01-07', 1_009_800, 4076.8], // 基准 −2%，策略 −1%
          ['2026-01-08', 1_029_996, 4239.872], // 基准 +4%，策略 +2%
        ]),
      })
    )
    expect(clean.beta).not.toBeNull()
    expect(clean.beta!).toBeCloseTo(0.5, 6)

    // 中间那天基准缺失：策略那一侧必须同步丢掉那两期，否则 beta 会被错位算成别的数
    const holed = summarize(
      base({
        equity: equity([
          ['2026-01-05', 1_000_000, 4000],
          ['2026-01-06', 1_020_000, 4160],
          ['2026-01-07', 1_009_800, null],
          ['2026-01-08', 1_029_996, 4239.872],
        ]),
      })
    )
    // 只剩第一期可配对 ⇒ 单期算不出协方差 ⇒ null（而不是一个由错位得来的数）
    expect(holed.beta).toBeNull()
  })
})

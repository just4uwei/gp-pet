 ...
 * 5. **两边的窗口对不对得上**要在动手之前就摆出来（截止日之内我们算多少、
 *    之后还有几笔没算）—— 这个功能最容易出的错是「你填的数含今天、我们排掉了今天」，
 *    而它出错时账面上没有任何异样。
 * 6. **逐笔对照必须摆出来** —— 合计是反解出来的，按构造总能对上，
 *    分辨不出「免最低勾了没有」这种配置错误。只有逐笔分得开，而用户手上正好有那张账单。
 */
  ...
  test('页内两步确认 · 零原生模态框 · 应用完还能立刻再校正一次', async () => {
    ...
    */
    ...
    /*
      ...
      截止日排掉了今天那几笔，而用户抄的券商数字已经把它们算进去了
      ⇒ 差额被整个记到佣金率头上。所以「截止日之内我们算多少」与
      「之后还有几笔没算」都要在他动手之前就摆出来。
    */
    await expect(panel.getByText(/截止日之内 \d+ 笔/)).toBeVisible()
    await expect(panel.getByText(/你填的那个数要是同一个窗口的/)).toBeVisible()
    ...
    await panel.getByRole('button', { name: '反解' }).click()
    /*
      ⑥ **逐笔对照必须摆出来。** 合计是反解出来的，按构造总能对上 ⇒ 它分辨不出
      配置对不对（真机：同一个 85.11，「免最低」勾与不勾都对得上合计，
      而逐笔一个 8/8 全错、一个 8/8 零残差）。用户手上正好有那张逐笔账单。
    */
    await expect(panel.getByText(/拿它对一下券商的逐笔账单/)).toBeVisible()

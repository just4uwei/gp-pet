...
| 改 AI 解读 | [src/main/ai/index.ts](./src/main/ai/index.ts) 头注释（五条纪律）+ [src/main/ai/service.ts](./src/main/ai/service.ts)（两层缓存 / 只有 done 落库）+ [AiDrawer.tsx](./src/renderer/panel/AiDrawer.tsx) 头注释（为什么单独一层）+ [docs/08 §后续](./docs/08-开发路线图.md) |
...
- **AI 解读自己一层抽屉，状态挂在 `App`，别把它挪回信号列表里**（2026-08-14）。
  内嵌在信号行里时，列表的重渲染能摘掉一次正在跑的模型调用：每组只渲染 `group.latest`，
  同一只票再来一条信号组头就换人，正在流式生成的那条从列表里消失 → 组件卸载 →
  `ai:cancel` → 等了四十秒的界面自己没了，而那次调用已经计过费。
  抽屉与列表**没有父子关系**，这是结构性保证，不是小心翼翼。
  （`pinnedSignal` 仍然留着，但理由换成了「正读着的那行不该凭空消失」，与 AI 无关。）
- **AI 解读的三条钱相关的纪律**（`ai_explain` 表 + `AiService`，2026-08-14）：
  ① **打开抽屉不等于发请求** —— `explain()` 按「内存缓存 → `ai_explain` 表 → 真发」找，
  别为了「打开就有反应」把查库那层去掉，那会让翻一次历史重复计费一次；
  ② **关抽屉不取消**（请求在主进程跑完并落库，重开时靠在途去重接上），
  **只有「停止」按钮真的 `ai:cancel`** —— 卸载即取消那条老纪律已经删掉了，别加回来；

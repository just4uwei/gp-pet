...
- **提醒日志当判据之前，先 `grep -c "窗口与托盘就绪"` 数那天重启过几次。**
  冷却/配额/台阶全在内存里，重启即清零 ⇒ 同一条止损重新发一次。
  实测 08-13 启动 14 次、08-14 二十七次（盘中 12 次），那两天「一天 10 条止损气泡」
  逐条对得上启动时刻。**M3 的自用一周从 2026-08-17 起算**（见 M3 清单 §4.0）。
...
- **提醒日志当判据之前，先数那天重启过几次**：
  ...
  冷却/配额/台阶全在内存里，重启即清零 ⇒ 同一条止损会重新发一次。

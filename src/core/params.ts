 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 * 下面这张是**另一回事**，别与上面那张混起来：已经上过标定网格、裁决 `KEEP`、
 * 因此**保持原值**的参数（M2 验收清单 4.9c）。「测过了，没有更好的」不等于
 * 「已标定」—— 它们的值仍然是转述猜测，只是我们知道往邻域动会更差或没差别。
 * 记在这里是为了**别再重跑同一张网格**；细节与失效条件见偏差报告。
 *
 * - `combine` 四数（`scoreThreshold` / `voteThreshold` / `downtrendBuyPenalty` /
 *   `conflictBand`）——「怎么动都测不出差别」，15 个候选最大 t = 1.4（§5.14 / §5.15）。
 *   其中 `downtrendBuyPenalty` 的幅度与 `conflictBand` 逐折动 0/12 与 2/12，属参数惰性。
 * - `adx.baseThreshold = 20` / `adx.volScale = 8` —— **有正面证据**：±2 与 ±4 的邻域
 *   全部更差，其中两个邻居让六年训练集亏钱被红线淘汰（§5.16 / §5.17）。
 * - `adx.maxThreshold = 28` —— 上侧**算术无效**（clamp 的上界永远咬不住，§5.16 有推导）；
 *   下侧 24 / 26 更差。往上调这个数不会改变任何行为，别扫。
 * - `regime.hysteresisDays = 2` —— 1 明显更差（Δ −0.34pp）；**2–6 分辨不出**
 *   （验证 Calmar 0.427 / 0.508 / 0.485 / 0.580 / 0.599，但 Δ 全部 ≤ +0.07pp、t ≤ 0.6）；
 *   8 开始转差（0.401 / Δ −0.08pp），10 被衰减红线标记，12 被验证集红线淘汰（§5.17）。
 *   若 M3 为「少改口」要上调，判据是提醒日志的每日改口次数、不是回测 Calmar，**且不要越过 6**。
 * - `regime.rangeMidBand = 0.03` —— 四个邻域取值全负但非单调、t ≤ 1.6，是噪音的形状（§5.17）。
 * - `strategy.revertLookback = 3` —— 参数惰性，根因在组合层的票数线不在取值（§5.11）。
 */
 ...
 *
 * **0.2.8 是第一次判定逻辑改动**（2026-08-15），不是调参：
 * `T3_BREAKOUT:BUY` 在 `TREND_UP` 状态下不再计票（docs/04 §3.1a 论证、
 * docs/07 §3.6 验收门槛、M2 §5.24 实测）。
 *
 * 与前面几次递增的**性质不同**，所以单独说清楚：
 * 0.2.5 是写回一个参数、0.2.6 是修一个口径 bug、0.2.7 是加一个参数块，
 * 而这一次**改的是「哪些信号成立」本身** —— 交易的集合变了，
 * 因此历史回测报告与新报告**不可逐笔比较**（配对 Δ 那套判据的前提就是两边测同一批东西）。
 * 依据是 §5.21/§5.23 的随机基准：TREND_UP 的入场在 200 次随机试验里排第 3–4 分位，
 * 而 `T3` 在该状态下与恒成立的 `T4` 几乎同现、不提供区分度。
 *
 * `-unvalidated` 仍然留着：这次改的是逻辑不是参数，62 个叶子里
 * 仍有 21 个 `BLOCKED` + 0 个已标定的新增。
 */
export const ENGINE_VERSION = '0.2.8-unvalidated'

...
export interface Position {
  ...
  stopFloor?: number
  /**
   * 这些股里「**今天买进的**」有多少 —— A 股 T+1，它们今天卖不掉。
   * 缺省 = 0（今天没买过）。由主进程按成交流水算（`trades.boughtSharesSince`）。
   *
   * 判据只作用于「做不到」这一类：卖出/减仓信号在**全仓都是今天买的**时候
   * 被硬抑制（与「已跌停，卖不掉」同一档），部分锁定时只标注；
   * 做T建议的「有没有底仓」也改判**可卖**股数（T+0 只能用老仓做）。
   *
   * ⚠ **回测与影子运行绝不设这个字段**，理由与 `stopFloor` 完全相同：
   * 它是用户这一次具体建仓的事实，不是策略的一部分。缺省即天然不受影响，
   * 两边的绩效数字因此逐位不变 —— **别去「补上」**。
   */
  lockedShares?: number
}

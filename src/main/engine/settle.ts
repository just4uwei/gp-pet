 ...
 *
 * ## 目标日只有两种：上一个交易日，或**当日且已确认补齐**；更早的不追
 *
 * 调用方（`engine/tick.ts` 的 `settleDue`）用 `expectedLastBar()` 给出目标日，
 * 它最多回到上一个交易日。**往前追等于给应用没开机的那些天凭空造出一份信号历史** ——
 * 那些信号从来没有出现在任何界面上、也从来没有经过闸门，
 * 却会让「今天引擎给了几条信号」这类统计再也答不准（与「观察点命中不写进 signal 表」同一条）。
 *
 * **2026-09-02 加了「当日」这一种**（用户拍板「补齐才提前」）：15:10–16:00 的收尾窗口里
 * 当日收盘线**一只不缺**地补齐时，同一跳就把当天跑掉，不必等到次日盘前。
 * 三条别读错：
 * - **这不破前向纪律。** 本模块用的仍是 `getContextThrough(code, D)`（截至 D 的收盘线、
 *   不拼临时线、不带快照），与次日跑出来的输入**逐位相同** —— 变的只是「什么时候跑」。
 * - **它不放宽「补齐」的判据。** 补不齐的日子什么都不写（连 `lastSettledDate` 都不写）
 *   ⇒ 次日盘前照旧补跑。放宽成「试到底就算数」会写下两道幂等闸门、挡掉次日那次完整补跑
 *   —— 那个版本**被明确否掉了**，别顺手改回去。
 * - **影子在这条路上恒喂**（边界 2 的判据在这条路上取 true）：走到那里必然是收盘之后，
 *   下一个开盘还没到。顺带解掉老路那个隐性条件「用户得在次日 09:30 前开着应用」。
 *

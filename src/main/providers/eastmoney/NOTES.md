...
角色：主源。日线（含复权）、批量快照、基础信息、交易日历、当日分时。
...
| 当日分时 | `GET https://push2his.eastmoney.com/api/qt/stock/trends2/get` |
...
**最后核对：2026-08-14**（真实响应，与腾讯 minute 交叉对过）——
这是本文件里唯一有真机依据的一段，上面那个「待核对」的告示不适用于它。

```
?secid=1.600000&ut=…&fields1=f1,f2,f3,f4,f5,f6,f7,f8
&fields2=f51,f53,f56,f58&iscr=0&ndays=1
```

- `data.trends` 是 CSV 字符串数组，列序同样**由 fields2 决定**：

| 列 | 字段 | 含义 |
|---|---|---|
| 0 | f51 | 时刻 `YYYY-MM-DD HH:mm`，**北京时间** |
| 1 | f53 | 该分钟收盘价 |
| 2 | f56 | **这一分钟**的成交量，手（分时图不画量，留着是为了列序对得上） |
| 3 | f58 | 当日均价，**接口直接给**，不用自己除 |

> 后两列与腾讯 minute 的后两列**含义完全不同**：那边是「当日累计量」与「当日累计额」，
> 均价要自己除。两边的解析器因此长得不一样，不是其中一个写错了。
> 实测同一分钟（08-14 09:31）两边算出的均价都是 `9.132`，可以用来交叉验证。

- `data.preClose` 是昨收 —— 分时图的基准线与右轴涨跌幅全靠它。
- **`iscr=0`** 不含集合竞价那一段（09:15–09:25 是虚价，docs/03 §3 的 AUCTION 一行）。
- **`ndays=1` 给的是「最近一个交易日」，休市时就是上一个交易日。**
  这不是错误，但**不许用本机日期去标它** —— 周六打开面板会得到一张
  日期错位、图上却毫无破绽的假曲线。`tradeDate` 因此取自数据本身的第一行。
- 午休那段本来就没有点，解析器**不补**，由渲染层断线（见 IntradayChart）。

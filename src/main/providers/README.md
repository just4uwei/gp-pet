...
| Provider | 角色 | 承担能力 |
|---|---|---|
| `eastmoney` | 主源 | 日线（含前复权）、批量快照、基础信息、交易日历、当日分时 |
| `sina` | 备源 | 批量快照 |
| `tencent` | 备源 | 快照、日线、当日分时 |
...
4. **自我限制**：全局并发 ≤ 4、单源并发 ≤ 2、快照必须批量（分片 ≤ 50 只）、日线只做增量、休市不发请求。见 [docs/03 §2.4](../../../docs/03-数据源与存储设计.md)。
   **`minute` 是唯一的例外形状**：它不进轮询，只由用户打开抽屉「行情」页时触发，
   闸门是它自己那道 30s 缓存（`engine/intraday.ts`）。**不许从 tick 里调它** ——
   那会把「一次一只票」变成「每 30 秒一整个自选列表」，是上面那份预算的几十倍。

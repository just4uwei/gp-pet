 ...
 * 但**没有持仓时**它们走 `OBSERVE` 轨提醒（独立日配额、抢不到气泡，见 `alerts/track.ts`）——
 * 目的是攒一份「行业 ETF 上的信号质量 vs 个股」的对照数据，而不是多一个打扰源。
 *
 * ⚠ **「不设持仓」那条 2026-08-18 已经取消**：用户可以在这些 ETF 上真的建仓，
 * 面板给「仓」入口，而**有持仓即按个股待遇**（提醒翻回 PRIMARY 轨、进日报、进悬浮条）。
 * 判据不在这个文件里 —— 分组只回答「它是不是内置的那 15 只」。
 *
 * 面板左栏的分屏也**不按这个分组**：那两屏分的是板块（`@shared/watch-split`），
 * 用户自己加的 ETF 与这 15 只在同一屏，区别只在能不能移除。
 *
...
/**
 * 「行业ETF」分组名。现在只回答一件事：**这一只是不是内置的那 15 只**。
 *
 * 用它的地方有三处，都不是「分 tab」：不可移除（删了下次启动会被补回来）、
 * 无持仓时走 OBSERVE 轨、无持仓时不进日报。左栏分屏 2026-08-18 起按板块走。
 */

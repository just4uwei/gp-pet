...
export interface PositionView {
  ...
  /**
   * 现价已经触及固定止损线（2026-08-15）。**由主进程算**，判据与 `src/core/risk`
   * 逐字一致：有用户画的线就比那个价，没有就比 `risk.stopLossPct`。
   *
   * 界面据此决定要不要给「重画止损线」的入口 —— **还没跌破的票不给**，
   * 那时提这件事只是噪音。缺省（含拿不到报价时）= 没跌破：
   * 不知道就不点亮入口，而不是拿未知去点亮它。
   *
   * ⚠ **渲染层不许自己算这个** —— `risk.stopLossPct` 在 `src/core/params.ts`，
   * 而 `renderer → core` 是禁止的。抄一个 0.08 出去的症状是
 ...
 *
 * ## ⚠ 这里**只有券商那两项**，印花税与过户费不在其中（2026-09-03 订正）
 *
 * 那两项是**国家与交易所的规定**，不是券商报价、也不是用户设置 ——
 * 它们住 `backtest/costs.ts` 并**带生效日期**（`stampTaxRateOn` /
 * `TRANSFER_FEE_RATE`），与 `priceLimitRatio` 那条「规则类常量要带生效日期」同一条纪律。
 *
 * 之前把印花税当成一个可配的数写死 0.001，而它 **2023-08-28 起已经减半到 0.0005**
 * ⇒ 用户账本上每一笔卖出都被多扣一倍（真机实测单只票多收 29.21 元），
 * 更坏的是它**污染了反解**：那 2 倍的误差被折算成「你的佣金是万 1.13、还免最低」，
 * 一个看起来精确、实际完全虚构的结论。**规则不该出现在设置里**，这就是理由。
 */
export interface TradeFeeRates {
  /** 佣金率（双边）。券商说的「万 2.5」= 0.00025。**唯一会被反解改动的一项** */
  commissionRate: number
  /**
   * 单笔最低手续费，元。**0 是合法的**（有券商免最低），不是「没填」。
   *
   * ⚠ 它卡的是**佣金 + 过户费**这一整块，不是只卡佣金 —— 真机账单：
   * 单笔 19,346 元的买入券商收 5.00 元整，只卡佣金的话应当是 5.19。
   */

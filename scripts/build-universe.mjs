 ...
 *
 * ## 幸存者偏差：这个池子有，但它**不再是无解的**（2026-08-17 更新）
 *
 * 本文件产出的池子只含**今天仍在交易**的股票（全集来自新浪当前行情列表），
 * 所以它自带幸存者偏差，方向是**绩效被系统性高估**。这一点没变。
 *
 * 变的是那句结论。这里原先写着「已退市的股票在**任何免费行情接口上都取不到日线**，
 * 因此不可能进样本」—— **那句话是错的**：腾讯 `fqkline` 有退市股的完整日线，
 * 一直延伸到各自的退市日，两轨都在，`fetch-history.mjs` 一个字不用改就能拉。
 * 真正卡住的一直是**名单**（新浪的 `delisting` / `tuishi` 节点实测全空），
 * 而名单也不需要新数据源：巨潮的 `szse_stock.json`（本仓库抓公告时已经在用）含退市股。
 *
 * 补法见 [build-delisted.mjs](./build-delisted.mjs)，产出 `params/universe-delisted.json`。
 * **本文件刻意不合并那批** —— 两个池子分开，「含退市 vs 不含退市」才能作为对照跑，
 * 而那个差值就是幸存者偏差的量化值。合成一个文件就再也拆不开了。
 */
    ...
    `${JSON.stringify(
      {
        ...
        knownBias:
          '本池只含今天仍在交易的股票 ⇒ 自带幸存者偏差，方向是绩效被系统性高估。' +
          '（2026-08-17 更正：此处原先写「退市股在免费接口上取不到日线，无法消除」—— 那句是错的，' +
          '腾讯有退市股完整日线，名单来自巨潮。补法见 params/universe-delisted.json 与 scripts/build-delisted.mjs；' +
          '两个池子刻意分开，「含退市 vs 不含退市」的差值就是幸存者偏差的量化值。）',
